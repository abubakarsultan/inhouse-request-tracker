    ...
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {projects.map((project: any) => (
          <article key={project.id} className="flex min-h-64 flex-col rounded-2xl border border-[var(--border)] bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      {projects.length === 0 && <div className="rounded-2xl border border-[var(--border)] bg-white p-8 text-center text-[var(--muted)]">No projects found.</div>}
    </div>
  );
}
