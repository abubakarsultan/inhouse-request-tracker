      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {CARDS.map(({ key, label, icon: Icon }) => {
          ...
          return key === 'failedSync' && stats.failedSync > 0 ? (
            <Link key={key} href="#failed-sync" className="rounded-2xl border border-[#f4b7b2] bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">{content}</Link>
          ) : (
            <div key={key} className="rounded-2xl border border-[var(--border)] bg-white p-5 shadow-sm">{content}</div>
          );
        })}
      </div>

      {stats.failedSync > 0 && (
        <section id="failed-sync" className="mt-6 rounded-2xl border border-[#f4b7b2] bg-white scroll-mt-6">
